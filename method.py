...
def prepare_input_data(exprs_file, network_file, verbose = True, min_n_nodes = 3):
    ...
    if verbose:
        ...
        print("time:\tInputs read in %s s." %round(time.time()-t0,2), file = sys.stdout)
...
def print_network_stats(G,print_cc = True):
    ...
    if print_cc and len(ccs)>1:
        ...
        for cc in ccs:
            i+=1
            print("Connected component",i,":",len(cc),"nodes and",len(nx.subgraph(G,cc).edges()),"edges", file = sys.stdout)
...
def load_network(infile_name, verbose = True):
    ...
    # read from file
    print("Loading annotated network from ",infile_name,"...",file = sys.stdout)
    ...
    try:
        ...
        pass
    if verbose:
        print("time: load_network() runtime", round(time.time()-t0,2),"s", file =sys.stdout)
    return network
...
def save_network(network,outfile_name, verbose = True):
    ...
    if verbose:
        print("time: save_network() runtime", round(time.time()-t0,2),"s", file =sys.stdout)
...
def sampling(network, edge2Module, edge2Patients,nOnesPerPatientInModules,moduleSizes,
    ...
    for step in range(1,max_n_steps):
        ...
        if n_times_cc_fulfilled == n_steps_for_convergence: # stop if convergence is True for the last n steps
            ...
            if verbose:
                print("time:\tSampling (%s steps) fininshed in %s s. Model converged." %(len(edge2Module_history),round(time.time()-t0,2)), file = sys.stdout)
            return edge2Module_history, n_final_steps,n_skipping_edges,P_diffs
    
    n_final_steps = n_steps_for_convergence
    if verbose:
        print("time:\tSampling (%s steps) fininshed in %s s. Model did not converge." %(len(edge2Module_history),round(time.time()-t0,2)), file = sys.stdout)
...
def genesets2biclusters(network,exprs_np, exprs_data,moduleSizes,consensus_edge2module,
    ...
    if verbose:
        ...
        print("Passed modules with >= 2 edges and >= %s samples: %s"%(min_n_samples,len(filtered_bics)), file = sys.stdout)
...
def merge_biclusters(biclusters, exprs, exprs_data, min_n_samples=5,
    ...
    if verbose:
        print("time:\tMerging (%s cycles, %s successful merges) fininshed in %s s." %(n_merging_cycles,n_merges,round(time.time()-t0,2)), file = sys.stdout)
        print("Modules remaining after merging:", len(merged_bics), file = sys.stdout)
    return merged_bics
